...
| 9   | 2026-09-25 | AI provider           | "Use the inline LLM" — to be confirmed whether this means a hosted (online) LLM API or an embedded model. | AI will sit behind a provider interface so the choice is configuration. **Open:** a hosted provider changes the PRD's "no data leaves the network" promise ([ADR 0006](../adr/0006-llm-provider-abstraction.md)). |

## Still open

- Confirm the AI provider (decision 9) and, if hosted, which provider and what data may be sent.
